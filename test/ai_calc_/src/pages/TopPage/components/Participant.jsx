...
const Participant = ({ deleteParticipant, participant, changeName, changeCount, changeWeight, checkWeight, updateForms }) => {
    ...
    return (
        <>
            <tr>
                <td className='col-1'><button onClick={deleteParticipant}>-</button></td>
                <td><input type="text" placeholder="役職または人名" value={participant.name} onChange={(e) => {changeName(e.target.value); setCheckInput(true);} } onBlur={(e) => {checkName(e.target.value)}}/></td>
                <td>
            ...
            { !checkInput && <tr><td></td><td style={{color: 'red'}}>職位は必須項目です。</td></tr> }
        </>
